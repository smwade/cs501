...
dropout = bernoulli(tf.shape(h1), prob)
...
dropout = bernoulli(tf.shape(h2), prob) 
...
dropout = bernoulli(tf.shape(h3), prob) 
...
dropout = bernoulli(tf.shape(h3), prob) 
h3 = tf.mul( dropout, h3)
y_hat = tf.nn.softmax(tf.mul(tf.matmul(h3, W4) + b4, scale))
